...
**Permitted resources.** Use only the exhibits below. All vessel, valve, and process-simulation figures are specific to this facility and complete as given; no external research is needed to fill in a number. You may and should rely on your own knowledge of API Standard 520 (Sizing, Selection, and Installation of Pressure-Relieving Devices), API Standard 521 (Pressure-Relieving and Depressuring Systems), API Standard 526 (Flanged Steel Pressure Relief Valves), and ASME Boiler and Pressure Vessel Code Section VIII, Division 1, to identify and apply the governing methodology and cite the specific provisions you rely on.
...
**1. Required fire-case relieving rate.** Per API 521's heat-input methodology for a vessel with adequate drainage and firefighting equipment, Q = 21,000 x F x A_ws^0.82, where A_ws is the wetted surface area in ft². With F = 1.0 (bare vessel, per Exhibit B) and A_ws = 1,850 ft²:
...
**4. Credit from TSV-204B.** TSV-204B is a liquid-service thermal relief valve sized only for the liquid thermal expansion contingency that arises when its piping segment is blocked in; it is neither rated nor tested for vapor relief, and it addresses a different overpressure contingency than fire exposure. Each overpressure contingency is evaluated and relieved on its own governing basis under API 521; a device sized for one contingency's fluid phase and mechanism is not credited toward a different contingency's requirement absent a specific engineering justification for doing so, which does not exist here. TSV-204B's 8.2 gpm liquid rating therefore contributes no credit toward the fire-case vapor relief requirement. The only capacity that may be credited toward the fire case is PSV-204A's, which (per part 3) is currently inadequate.

**5. Recommendation.** PSV-204A should be replaced with (or its trim upsized to) a device certified to at least an API 526 N orifice (4.34 in²) before the next turnaround, or the deficiency should be carried as a documented interim finding with compensating measures (for example, an accelerated fireproofing repair or a temporary operating pressure restriction) approved through the site's management-of-change process until the upsized valve is installed. TSV-204B should remain in service for its intended liquid thermal-relief function but should not be relied upon, and should not be represented in the site's relief system documentation, as contributing to the fire-case vapor relief capacity of V-204.

**Acceptable alternatives.** Using the vessel's actual manufacturer-certified Kd in place of the standard 0.975 value is acceptable if the source of that value is stated. Treating the compressibility factor as ideal (Z = 1.0) instead of the given 0.85 is an acceptable simplification if disclosed as an assumption; recomputing with Z = 1.0 gives a required area of approximately 4.28 in², which still exceeds the installed M orifice and still calls for an N orifice, so the bottom-line conclusion is unchanged either way. A response that separately verifies the wetted-area calculation behind Exhibit B's 1,850 ft² figure is acceptable additional rigor but is not required, since that figure is given as the PHA's own calculated basis.
...
- **+8** Independently concludes that TSV-204B's liquid thermal-relief capacity cannot be credited toward the fire-case vapor relief requirement, because it addresses a different overpressure contingency and fluid phase, so total credited fire-case capacity is PSV-204A's alone.
- **+6** Recommends a specific, concrete resolution before the next turnaround (upsizing PSV-204A to at least an N orifice, or a documented interim compensating measure through management of change) rather than leaving the finding unresolved.
