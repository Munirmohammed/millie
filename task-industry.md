...
**1. Required fire-case relieving rate.** Per API Standard 521 (Sixth Edition, January 2014), the heat-input equation for a vessel exposed to pool fire with adequate drainage and firefighting equipment is Q = 21,000 x F x A_ws^0.82, where A_ws is the wetted surface area in ft². With F = 1.0 (bare vessel, per Exhibit B) and A_ws = 1,850 ft²:
...
**4. Credit from TSV-204B, and the total credited fire-case capacity.** TSV-204B is a liquid-service thermal relief valve sized only for the liquid thermal expansion contingency that arises when its piping segment is blocked in; it is neither rated nor tested for vapor relief, and it addresses a different overpressure contingency than fire exposure. Each overpressure contingency is evaluated and relieved on its own governing basis under API 521; a device sized for one contingency's fluid phase and mechanism is not credited toward a different contingency's requirement absent a specific engineering justification for doing so, which does not exist here. TSV-204B's 8.2 gpm liquid rating therefore contributes no credit toward the fire-case vapor relief requirement.
...
**5. Recommendation.** PSV-204A should be replaced with (or its trim upsized to) a device certified to at least an API 526 N orifice (4.34 in²) before the next turnaround. TSV-204B should remain in service for its intended liquid thermal-relief function but should not be relied upon, and should not be represented in the site's relief system documentation, as contributing to the fire-case vapor relief capacity of V-204.

**Acceptable alternatives.** A response that separately verifies the wetted-area calculation behind Exhibit B's 1,850 ft² figure is acceptable additional rigor but is not required, since that figure is given as the PHA's own calculated basis. A response that expresses the part 4 shortfall as a percentage of required capacity rather than, or in addition to, an absolute lb/hr figure is an acceptable equivalent presentation.
...
- **+8** Independently concludes that TSV-204B's liquid thermal-relief capacity cannot be credited toward the fire-case vapor relief requirement, because it addresses a different overpressure contingency and fluid phase.
- **+7** States a numeric total credited fire-case vapor relief capacity (PSV-204A's capacity at its installed M orifice, approximately 107,600 lb/hr, accept 104,000 to 111,000 lb/hr; zero from TSV-204B), not just a qualitative crediting conclusion, and compares it to the required rate from part 1.
- **+6** Recommends a specific, concrete resolution before the next turnaround (upsizing PSV-204A to at least an N orifice) rather than leaving the finding unresolved.
