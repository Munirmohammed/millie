...
## 4d-2. Real QC finding patterns worth designing around up front

Learned by actually clearing findings on task `92d565d9ac2063859359`, none of them platform bugs - genuine content defects. Check for all of these before ever requesting checks:

- **Every citation the reference/prompt claims must resolve to a locator someone could actually verify, not a paraphrase of the topic.** If a real search-and-fetch effort still can't find a specific clause/section number for a standard you know is real (this happens - some standards are paywalled with no secondary source quoting exact numbers), don't invent one. Cite what you can verify (the standard and edition, the exact equation/rule text) and stop there - and correspondingly soften the prompt's own "cite the specific provision behind every step" language so it doesn't demand precision you can't supply. A claim you can't back is worse than an honestly hedged one.
- **A conclusion attributed to "under [Standard X]" needs either a real locator for that specific sentence, or it needs to stop being attributed to the standard.** A generally-true, defensible piece of engineering/professional judgment (for example, "a device isn't credited toward a contingency it wasn't designed for") is fine to state as reasoning or established practice - it becomes a finding only when the reference dresses it up as if a specific numbered provision says exactly that and no such provision was actually located.
- **Never tell the respondent "no external research is needed for any number" if solving the task actually requires knowing a standard reference table or formula they'd otherwise have to look up.** Split the "allowed tools" language into two explicit buckets: facility/scenario-specific figures (supplied-only, verify this is really true) versus general industry reference knowledge the respondent is expected to already know (name it explicitly as such). Getting this boundary blurry is a solvability defect, not a minor wording issue.
- **A rubric criterion for "make a recommendation" should accept every response that actually solves the underlying problem, not just the one specific fix you happened to write first.** If more than one real engineering/professional path closes the same gap (upsize a component, add a second one, remove the root cause, etc.), the rubric and reference need to say so explicitly, or a respondent with an equally valid different fix gets penalized for it.
...
- **A rubric criterion's "Depends on" field defaults to something bad (observed: the criterion's own id, a self-reference) if it is never explicitly filled, and the autofill extension could not have fixed this on its own.** The extension originally treated an absent/empty value as "leave this field alone," which meant any criterion with no real dependency just kept whatever the platform had defaulted it to - including a self-reference that content QC flags as a scoring cycle. Fixed in `content.js`: an explicit empty string (`""`) in the data file now actively clears the field, while only `undefined`/missing still means "don't touch it." Every rubric criterion's data should therefore set `dependsOn` (and `followThroughOf`) explicitly - a real list of ids where one is intended, `""` everywhere else - never just omit the field and assume it's harmlessly blank on the platform side. Since this could affect any criterion silently until QC happens to flag it, checking every criterion at once (as done here) is safer than waiting for QC to surface them one at a time.
